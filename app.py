...
class Mygarden:
    ...
    def add_widgets(self):
        ...
        self.btn1 = ttk.Button(self.frame1, text="Inventory", command=self.test_command)
        self.btn1.config(width=14)
        self.btn1.grid(padx="10", pady="5", row=0, column=0)
        #---------
        self.btn2 = ttk.Button(self.frame1, text="Scan", command=self.scanner)
        self.btn2.config(width=14)
        self.btn2.grid(padx="10", pady="5", row=0, column=1)
        #---------
        # self.btn3 = ttk.Button(self.frame1, text="Print codes", command=self.print_codes)
        ...
        self.gardenframe = tk.LabelFrame(self.master, text="MyGarden")
        self.gardenframe.grid(padx=5, pady=5, ipadx=2, ipady=2, sticky=tk.N, row=1, column=0)

        self.slotsnx = 5
        self.slotsny = 4
        self.slots = np.zeros((self.slotsnx, self.slotsny))
        for i in range(self.slotsnx):
            for j in range(self.slotsny):
                self.slots[i, j] = tk.Frame(self.gardenframe, bg="black", height=100, width=100)\
                    .grid(padx=1, pady=1, ipadx=1, ipady=1, row=j, column=i)
